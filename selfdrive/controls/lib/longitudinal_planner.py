...
_A_CRUISE_MIN_V = [-1.5, -1.2, -.67, -.5, -.30]
# _A_CRUISE_MIN_V = [-1.15, -.85, -.7, -.55, -.32]
_A_CRUISE_MIN_BP = [  0.,  5.,  10., 20.,  40.]

# need fast accel at very low speed for stop and go
# make sure these accelerations are smaller than mpc limits
#_A_CRUISE_MAX_V = [1.2, 1.2, 0.8, .8]    # Last values
_A_CRUISE_MAX_V = [1.6, 1.6, 0.8, .8]
#_A_CRUISE_MAX_V = [1.2, 1.2, 0.65, .4]
_A_CRUISE_MAX_V_FOLLOWING = [1.6, 1.6, 0.8, .8]
...
def limit_accel_in_turns(v_ego, angle_steers, a_target, CP):
  ...
  a_x_allowed = math.sqrt(max(a_total_max**2 - a_y**2, 0.5))
